 ...
 */
static char *
und_alloc (struct parsed_symbol *sym, size_t len)
{
  void *ptr;
 ...
 */
static void
und_free_all (struct parsed_symbol *sym)
{
  void *next;
 ...
 */
static void
str_array_push (struct parsed_symbol *sym, const char *ptr, int len, struct array *a)
{
  assert (ptr);
  assert (a);
    ...
    for (i = a->max; i > 0; i--)
      {
        ...
        if (i - 1 < a->start)
          ...
          c = '}';
        TRACE ("%p\t%d%c %s", a, i - 1, c, a->elts[i - 1]);
      }
  }
#endif
}
 ...
 */
static char *
str_array_get_ref (struct array *cref, unsigned idx)
{
  assert (cref);
 ...
 */
static char *
str_printf (struct parsed_symbol *sym, const char *format, ...)
{
  va_list args;
  size_t len = 1, i, sz;
  char *tmp;
...
static const char *
get_number (struct parsed_symbol *sym)
{
  char *ptr;
  bool sgn = false;
 ...
 */
static char *
get_args (struct parsed_symbol *sym, struct array *pmt_ref, bool z_term, char open_char, char close_char)
 ...
 */
static bool
get_modifier (char ch, const char **ret)
{
  switch (ch)
    {
...
static bool
get_modified_type (struct datatype_t *ct, struct parsed_symbol *sym, struct array *pmt_ref, char modif, bool in_args)
{
  const char *modifier;
 ...
 */
static char *
get_literal_string (struct parsed_symbol *sym)
{
  ...
  do
    if (!((*sym->current >= 'A' && *sym->current <= 'Z') ||
      ...
      {
        TRACE ("Failed at '%c' in %s", *sym->current, ptr);
        return NULL;
      }
  while (*++sym->current != '@');
  sym->current++;
 ...
 */
static char *
get_template_name (struct parsed_symbol *sym)
{
  char *name, *args;
 ...
 */
static bool
get_class (struct parsed_symbol *sym)
{
 ...
 */
static char *
get_class_string (struct parsed_symbol *sym, unsigned int start)
{
  unsigned int i;
  size_t len, sz;
  char *ret;
 ...
 */
static char *
get_class_name (struct parsed_symbol *sym)
{
  unsigned mark = sym->stack.num;
  char *s = NULL;
 ...
 */
static bool
get_calling_convention (char ch, const char **call_conv, const char **exported, unsigned flags)
{
          ...
          switch (ch)
            ...
            case 'M':
              *call_conv = "clrcall";
              break;
            default:
              ERR ("Unknown calling convention %c", ch);
              return false;
            }
        }
      else if (((ch - 'A') % 2) == 1)
 ...
 */
static bool
demangle_datatype (struct parsed_symbol *sym, struct datatype_t *ct, struct array *pmt_ref, bool in_args)
{
  char dt;
  bool add_pmt = true;
  ...
  switch (dt = *sym->current++)
    ...
    case '?':
      ...
      if (in_args)
        {
          const char *ptr;
          if (!(ptr = get_number (sym)))
            goto done;
          ct->left = str_printf (sym, "`template-parameter-%s'", ptr);
        }
      else if (!get_modified_type (ct, sym, pmt_ref, '?', in_args))
        goto done;
      break;
    case 'A': /* reference */
    case 'B': /* volatile reference */
      if (!get_modified_type (ct, sym, pmt_ref, dt, in_args))
        goto done;
      break;
    ...
    case 'P': /* Pointer */
      ...
      else if (!get_modified_type (ct, sym, pmt_ref, 'P', in_args))
        goto done;
      break;
    case 'W':
      if (*sym->current == '4')
        ...
        case 'F':
          {
            const char *p1;
            const char *p2;
            if (!(p1 = get_number (sym)))
              goto done;
            if (!(p2 = get_number (sym)))
              goto done;
          ...
          if (*sym->current == 'C')
            {
              ...
              sym->current++;
              if (!get_modifier (*sym->current++, &ptr))
                goto done;
 ...
 */
static bool
handle_data (struct parsed_symbol *sym)
{
    ...
    case '5':
      {
        ...
        if (!demangle_datatype (sym, &ct, &pmt, false))
          goto done;
        if (!get_modifier (*sym->current++, &modifier))
          goto done;
        sym->stack.num = mark;
  ...
  if (sym->flags & UNDNAME_NAME_ONLY)
    ct.left = ct.right = modifier = NULL;
 ...
 */
static bool
handle_method (struct parsed_symbol *sym, bool cast_op)
{
  char accmem;
  const char *access = NULL;
  const char *member_type = NULL;
  ...
  else if (!demangle_datatype (sym, &ct_ret, &array_pmt, false))
    goto done;
  ...
  mark = sym->stack.num;
  if (!(args_str = get_args (sym, &array_pmt, true, '(', ')')))
    goto done;
  if (sym->flags & UNDNAME_NAME_ONLY)
    args_str = modifier = NULL;
  sym->stack.num = mark;
 ...
 */
static bool
handle_template (struct parsed_symbol *sym)
{
  const char *name;
  const char *args;
  ...
  if (!(name = get_literal_string (sym)))
    return false;
  if (!(args = get_args (sym, NULL, false, '<', '>')))
    return false;
 ...
 */
static bool
symbol_demangle (struct parsed_symbol *sym)
{
  bool ret = false;
  unsigned do_after = 0;
          ...
          switch (*++sym->current)
            ...
            case 'R':
              ...
              switch (*++sym->current)
                ...
                default:
                  ERR ("Unknown RTTI operator: _R%c", *sym->current);
                  break;
                }
              break;
            ...
            default:
              ERR ("Unknown operator: _%c", *sym->current);
              return false;
            }
          break;
        default:
          /* FIXME: Other operators */
          ERR ("Unknown operator: %c", *sym->current);
          return false;
        }
      sym->current++;
      switch (do_after)
 ...
 */
static char *
unDNameEx (char *buffer, const char *mangled, int buflen, malloc_func_t memget, free_func_t memfree, void *unknown, unsigned short int flags)
{
  ...
  if (buffer && buflen)
    ...
    {
      buffer = static_cast<char *> (memget (strlen (result) + 1));
      if (buffer)
 ...
 */
char *
unDName (char *buffer, const char *mangled, int buflen, malloc_func_t memget, free_func_t memfree, unsigned short int flags)
{
  return unDNameEx (buffer, mangled, buflen, memget, memfree, NULL, flags);
}
