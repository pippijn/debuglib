...
template<typename T>
static void
checked (char const* file, int line, char const* operation, T retval)
{
  if (retval == -1)
    {
...
request_data (layout& bp, word_t addr)
{
  {
    word_t data;
    xassert (request_data (data, addr + offsetof (layout, xbp)));
    force_assign (bp.xbp, data);
  }
  {
    word_t data;
    xassert (request_data (data, addr + offsetof (layout, ret)));
    force_assign (bp.ret, data);
  }
}
...
backtrace (word_t lastbp, word_t lastesp)
{
  ...
  if (addrs.empty ())
  ...
  else
    {
      ...
      frame* frames = backtrace_frames (&addrs[0], (int)addrs.size ());
